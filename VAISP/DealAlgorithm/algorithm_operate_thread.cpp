...
void AlgorithmOperate::temperatureCalculate()
{
	...
	try
	{
		...
		{
			std::copy(T1.begin() + m_nAxleMarkG2[2 * i]+250, T1.begin() +  m_nAxleMarkG2[2 * i+1]+250, std::back_inserter(bearPalteSignalDivided[i]));
			...
			bearPalteTemperatureDivided[0].push_back((averageTemperature * 50)+3);//存入温度
			...
			bearPalteTemperatureDivided[1].push_back((maxTemperature * 50)+2);
			bearPalteTemperatureDivided[2].push_back(std::fabs(((environmentTemperature - maxTemperature) * 50))+3);
...
void AlgorithmOperate::electricalTemperatureCalculate()
{
	...
	for (long int i = 0; i < 8; i++)
	{
		...
		electricalTemperatureDivided[2].push_back(std::fabs((maxValue - environmentTemperature) * 50));
...
void AlgorithmOperate::speedCalculate()
{
	try
	{
		...
		double gapTime = abs(distancePos2 - distancePos1) / 10000;
...
void AlgorithmOperate::vibrationCalculate()
{
	...
	logHelper->writeError("algoritmRunLog", QStringLiteral("vibrationCalculate:计算完成, 位置标记：") + QString::number(m_nFlag));
