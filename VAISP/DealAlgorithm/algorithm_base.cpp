...
void AlgorithmBase::analysisAndSaveData()
{
	...
	for (size_t i = 0; i < allGrabData.size() / 16; i++)
	{
		...
		g4.push_back(allGrabData[i * 16 + 8]); g1.push_back(allGrabData[i * 16 + 9]); g3.push_back(allGrabData[i * 16 + 10]);
		g2.push_back(allGrabData[i * 16 + 11]);

		t4.push_back(allGrabData[i * 16 + 12]); t3.push_back(allGrabData[i * 16 + 13]); t2.push_back(allGrabData[i * 16 + 14]);
		t1.push_back(allGrabData[i * 16 + 15]);
	}
	...
	std::vector<double> &t1, std::vector<double> &t2, std::vector<double> &t3, std::vector<double> &t4)
{
	...
	QtConcurrent::run([=](){
		...
		{
			m_bIsRunAlg = true;
			algotrithOperateRight->initialData(v5, v6, v7, v8, g2, g3, g4, t2, t4, sensitivity2, 1);
			...
			algotrithOperateLeft->initialData(v1, v2, v3, v4, g2, g3, g4, t1, t3, sensitivity1, 0);
